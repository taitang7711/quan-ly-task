    ...
    <div class="auth-container">
      <div class="auth-card glass-strong">
        <div class="text-center mb-6">
          <div class="flex items-center justify-center gap-2 mb-3">
            <div class="w-10 h-10 rounded-xl gradient-bg flex items-center justify-center shadow-lg shadow-blue-900/20">
              <v-icon color="white" size="24">mdi-account-plus-outline</v-icon>
            </div>
          </div>
          <h1 class="text-3xl font-extrabold gradient-text">Task Manager</h1>
          <p class="text-gray-500 mt-2 text-sm">Tạo tài khoản mới</p>
        </div>
        ...
        <div class="text-center mt-6">
          <span class="text-gray-400 text-sm">Đã có tài khoản? </span>
          <router-link to="/login" class="text-blue-600 font-semibold text-sm hover:text-blue-700 transition-colors">Đăng nhập</router-link>
        </div>
      </div>
    </div>
  </div>
...
async function handleRegister() {
  ...
  try {
    ...
    show(error.response?.data?.error || 'Đăng ký thất bại', 'error');
  } finally {
    loading.value = false;
  }
}
